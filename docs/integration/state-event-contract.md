...
`COMPLETED/FAILED/CANCELLED` 为终态。任何晚到事件不得回退终态。
...
| 2 | Bridge 接收管理请求 | QUEUED | CREATED | HTTP 202；显示下发中 |
| 3 | Jetson heartbeat 领取 | LEASED | DISPATCHING | 仍下发中 |
| 4 | leaseToken 随命令返回 | LEASED | DISPATCHING | 无变化 |
| 5 | Jetson 持久化并 ACK | ACKED | DISPATCHING | 无变化 |
| 6 | Bridge 接收 ACK | ACKED | DISPATCHING | 无变化 |
| 7 | Jetson ROS publish | Jetson DISPATCHED | DISPATCHING | 无变化 |
| 8 | Jetson 持久化 DISPATCHED | Jetson DISPATCHED | DISPATCHING | 无变化 |
| 9 | Patrol 产生 `route_started` | 等待事件上传 | DISPATCHING | 无变化 |
