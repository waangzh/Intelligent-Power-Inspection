...
### 6.4 `POST /robot-api/v1/map-assets`

请求头：`Authorization: Bearer token-placeholder`、`Idempotency-Key: <UUID>`、`Content-Type: multipart/form-data`。设备身份只从 Token 获取，multipart 不接受 `robotId` 或 `siteId` 作为业务归属。

multipart 字段：

| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `yaml` | 是 | ROS 地图 YAML，最大 1 MiB |
| `pgm` | 是 | 8-bit P5 PGM，最大 100 MiB |
| `capturedAt` | 否 | 快照时间，UTC ISO-8601 |
| `contentIdentitySha256` | 是 | 下述规范化 JSON 的 SHA-256 |
| `yamlSha256` | 是 | 原始 YAML 字节审计哈希 |
| `pgmSha256` | 是 | 原始 PGM 字节哈希 |

内容身份不使用原始 YAML 哈希。Jetson 与 Spring 分别独立计算下列 JSON，键排序、无空白后做 SHA-256；浮点数使用无多余零的十进制字符串，缺失的 `negate/occupied_thresh/free_thresh/mode` 分别按 `0/0.65/0.25/trinary` 归一化：

```json
{"freeThresh":"0.25","mode":"trinary","negate":0,"occupiedThresh":"0.65","origin":["-2.89","-6.37","0"],"pgmSha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb","resolution":"0.05"}
```

Spring 只接受 `image/resolution/origin/negate/occupied_thresh/free_thresh/mode`，并返回自己计算的身份；Jetson 必须核对完全一致。
...
```json
{
  ...
  "contentIdentitySha256": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  "yamlSha256": "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc",
  "pgmSha256": "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
}
```

首次成功返回 `201`，相同设备、相同幂等键和相同内容身份返回 `200` 与原 `mapAssetId`。若原资产仍为 `PENDING_REVIEW` 但 YAML/PGM 缺失，同一请求可使用原幂等键修复文件并保留资产 ID；`REJECTED`、`AVAILABLE` 或内容不同的资产不可覆盖。`400/413`、`401/403`、同键不同内容的 `409` 为最终失败；处理中或暂时不可用返回 `503`，与 `429`、网络错误一样使用原幂等键指数退避。上传成功只进入 `PENDING_REVIEW`，不会自动改变路线、修订或部署。人工驳回不改变该次上传已经成功的事实；机器人发起新的上传意图时必须生成新幂等键。

### 6.5 `GET /robot-api/v1/deployments/{deploymentId}/manifest`
...
### 6.6 `GET /robot-api/v1/deployments/{deploymentId}/route`
...
### 6.7 `GET /robot-api/v1/deployments/{deploymentId}/yaml`
...
### 6.8 `GET /robot-api/v1/deployments/{deploymentId}/pgm`
