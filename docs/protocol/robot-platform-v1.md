...
首次成功返回 `201`，相同设备、相同幂等键和相同内容身份返回 `200` 与原 `mapAssetId`。`400/413`、`401/403`、同键不同内容的 `409` 为最终失败；处理中或暂时不可用返回 `503`，与 `429`、网络错误一样使用原幂等键指数退避。上传成功只进入 `PENDING_REVIEW`，不会自动改变路线、修订或部署。人工驳回不改变该次上传已经成功的事实；机器人发起新的上传意图时必须生成新幂等键。
