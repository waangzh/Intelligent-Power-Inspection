...
/**
 * Allow terminal ESTOPPED on inspection_tasks (distinct from CANCELLED). H2 uses DROP CONSTRAINT;
...
public class V23__task_estopped_status extends BaseJavaMigration {
  ...
  public void migrate(Context context) throws Exception {
    ...
    try (Statement statement = connection.createStatement()) {
      if (h2) {
        statement.execute(
            "ALTER TABLE inspection_tasks DROP CONSTRAINT chk_inspection_tasks_status");
      } else {
        try {
          statement.execute(
              "ALTER TABLE inspection_tasks DROP CONSTRAINT chk_inspection_tasks_status");
          ...
          try {
            statement.execute(
                "ALTER TABLE inspection_tasks DROP CHECK chk_inspection_tasks_status");
          } catch (Exception e2) {
