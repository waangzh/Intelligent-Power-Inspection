...
public class AgentActionProposalService {
  ...
  public AgentActionProposalService(AgentActionRepository actionRepository, AgentEvidenceRepository evidenceRepository, AgentPolicyService policyService, AgentActionPayloadValidator payloadValidator, AgentOrchestratorProperties orchestratorProperties, ObjectMapper objectMapper) {
    this.actionRepository = actionRepository; this.evidenceRepository = evidenceRepository; this.policyService = policyService; this.payloadValidator = payloadValidator; this.orchestratorProperties = orchestratorProperties; this.objectMapper = objectMapper;
  }

  public ProposalResult propose(AgentCaseEntity agentCase, AgentRunEntity run, ActionProposal raw, UserEntity user) {
    List<AgentEvidenceEntity> evidence = evidenceRepository.findByRunIdOrderByCreatedAtAsc(run.getId());
    ...
    AgentActionEntity existing = actionRepository.findByIdempotencyKeyOrderByCreatedAtAsc(key).stream().findFirst().orElse(null);
    if (existing != null) return new ProposalResult(existing, false);
    AgentPolicyDecision policy = policyService.evaluate(new AgentPolicyContext(user, agentCase, run, proposal, evidence, orchestratorProperties.isAutoExecutionEnabled()));
  ...
  public ActionProposal revalidatedProposal(AgentActionEntity action, AgentCaseEntity agentCase, Map<String, Object> payload) {
    List<AgentEvidenceEntity> evidence = evidenceRepository.findByRunIdOrderByCreatedAtAsc(action.getRunId());
    ActionProposal raw = new ActionProposal(action.getType().name(), action.getTitle(), action.getReason(), payload, evidenceIds(action), action.getConfidence());
    return payloadValidator.validate(raw, agentCase, evidence);
  }

  public AgentPolicyDecision policy(AgentActionEntity action, AgentCaseEntity agentCase, AgentRunEntity run, ActionProposal proposal, UserEntity user) {
    return policyService.evaluate(new AgentPolicyContext(user, agentCase, run, proposal, evidenceRepository.findByRunIdOrderByCreatedAtAsc(run.getId()), orchestratorProperties.isAutoExecutionEnabled()));
  }

  public String idempotencyKey(AgentEnums.ActionType type, String businessId, Map<String, Object> payload) { return type.name() + ":" + businessId + ":" + sha256(json(new TreeMap<>(payload))); }
  public List<String> evidenceIds(AgentActionEntity action) { try { return objectMapper.readValue(action.getEvidenceIdsJson(), STRING_LIST); } catch (Exception ex) { return List.of(); } }
  public Map<String, Object> payload(AgentActionEntity action) { try { return objectMapper.readValue(action.getPayloadJson(), MAP_TYPE); } catch (Exception ex) { return Map.of(); } }
  private String businessId(ActionProposal proposal, AgentCaseEntity agentCase) { Object alarmId = proposal.payload().get("alarmId"); if (alarmId != null && !alarmId.toString().isBlank()) return alarmId.toString(); Object taskId = proposal.payload().get("taskId"); if (taskId != null && !taskId.toString().isBlank()) return taskId.toString(); return agentCase.getId(); }
