...
public class AgentActionWorkflowService {
  ...
  public AgentActionEntity approve(String actionId, AgentDtos.ActionDecisionRequest request, UserEntity user) {
    ...
    if (policy.decision() == AgentEnums.PolicyDecisionType.DENY) {
      action.setStatus(AgentEnums.ActionStatus.REJECTED); action.setRejectedById(user.getId()); action.setRejectedAt(Instant.now()); action.setRejectionComment(policy.reason());
      action = save(action); recordStep(run, AgentEnums.StepType.ACTION_REJECTED, "策略拒绝了审批动作", Map.of("actionId", action.getId(), "policyCode", policy.policyCode())); syncState(agentCase, run); return action;
    }
    action.setStatus(AgentEnums.ActionStatus.APPROVED); action.setApprovedById(user.getId()); action.setApprovedAt(Instant.now()); action.setApprovalComment(request.comment().trim()); action.setUpdatedAt(Instant.now());
    action = save(action); recordStep(run, AgentEnums.StepType.ACTION_APPROVED, "动作已获人工批准", Map.of("actionId", action.getId(), "policyCode", policy.policyCode()));
  ...
  public AgentActionEntity retry(String actionId, AgentDtos.ActionDecisionRequest request, UserEntity user) {
    ...
    action.setStatus(AgentEnums.ActionStatus.APPROVED); action.setApprovalComment("重试：" + request.comment().trim()); action.setUpdatedAt(Instant.now()); action = save(action); recordStep(run, AgentEnums.StepType.ACTION_APPROVED, "失败动作已重新获准执行", Map.of("actionId", action.getId())); return execute(action, user);
  }

  @Transactional
  public AgentActionEntity autoExecute(AgentActionEntity action, UserEntity user) {
    if (action.getStatus() != AgentEnums.ActionStatus.PROPOSED || action.getPolicyDecision() != AgentEnums.PolicyDecisionType.AUTO_EXECUTE) return action;
    action.setStatus(AgentEnums.ActionStatus.APPROVED); action.setApprovedById(user.getId()); action.setApprovedAt(Instant.now()); action.setApprovalComment("策略自动批准：" + action.getPolicyReason()); action.setUpdatedAt(Instant.now()); action = save(action);
  ...
  private AgentActionEntity execute(AgentActionEntity action, UserEntity user) {
    ...
    AgentExecutionClaimService.ClaimResult claim = claimService.claim(action.getIdempotencyKey(), action.getId());
    if (!claim.owner()) {
      if (claim.claim().getStatus() == AgentEnums.ExecutionClaimStatus.SUCCEEDED) {
        action.setStatus(AgentEnums.ActionStatus.SUCCEEDED); action.setResultJson(claim.claim().getResultJson()); action.setExecutionCompletedAt(Instant.now()); action.setUpdatedAt(Instant.now()); action = save(action); recordStep(run, AgentEnums.StepType.ACTION_SUCCEEDED, "动作复用了已有幂等结果", Map.of("actionId", action.getId())); syncState(agentCase, run); return action;
      }
    ...
    syncState(agentCase, run); return action;
  }

  private void applyPayloadAndPolicy(AgentActionEntity action, ActionProposal proposal, AgentPolicyDecision policy, Map<String, Object> requestedPayload, UserEntity user) {
    if (requestedPayload != null && !requestedPayload.isEmpty() && !json(proposal.payload()).equals(action.getPayloadJson())) {
      action.setPayloadAuditJson(json(Map.of("before", proposalService.payload(action), "after", proposal.payload(), "changedBy", user.getId(), "changedAt", Instant.now().toString()))); action.setPayloadJson(json(proposal.payload())); action.setIdempotencyKey(proposalService.idempotencyKey(action.getType(), businessId(proposal, action), proposal.payload()));
    }
    action.setConfidence(proposal.confidence()); action.setPolicyDecision(policy.decision()); action.setPolicyCode(policy.policyCode()); action.setPolicyReason(policy.reason()); action.setRiskLevel(policy.riskLevel()); action.setRequiresApproval(policy.decision() == AgentEnums.PolicyDecisionType.REQUIRE_APPROVAL); action.setUpdatedAt(Instant.now());
  }

  private void syncState(AgentCaseEntity agentCase, AgentRunEntity run) {
    List<AgentActionEntity> runActions = actionRepository.findByRunIdOrderByCreatedAtAsc(run.getId());
    boolean executing = runActions.stream().anyMatch(item -> item.getStatus() == AgentEnums.ActionStatus.EXECUTING);
    boolean pending = runActions.stream().anyMatch(item -> item.getStatus() == AgentEnums.ActionStatus.PROPOSED && item.isRequiresApproval());
    boolean failed = runActions.stream().anyMatch(item -> item.getStatus() == AgentEnums.ActionStatus.FAILED);
    if (failed) { run.setStatus(AgentEnums.RunStatus.FAILED); run.setErrorCode("ACTION_FAILED"); run.setErrorMessage("受控动作执行失败"); run.setCompletedAt(Instant.now()); agentCase.setStatus(AgentEnums.CaseStatus.FAILED); }
    else if (executing) { run.setStatus(AgentEnums.RunStatus.ACTION_EXECUTING); agentCase.setStatus(AgentEnums.CaseStatus.ACTION_EXECUTING); }
  ...
  private AgentActionEntity save(AgentActionEntity action) { try { return actionRepository.saveAndFlush(action); } catch (OptimisticLockingFailureException ex) { throw ApiException.conflict("动作已被其他请求处理，请刷新后重试"); } }
  private String businessId(ActionProposal proposal, AgentActionEntity action) { Object alarmId = proposal.payload().get("alarmId"); if (alarmId != null) return alarmId.toString(); Object taskId = proposal.payload().get("taskId"); return taskId == null ? action.getCaseId() : taskId.toString(); }
  private void recordStep(AgentRunEntity run, AgentEnums.StepType type, String summary, Map<String, Object> detail) { AgentStepEntity step = new AgentStepEntity(); step.setId(Ids.next("agent_step")); step.setCaseId(run.getCaseId()); step.setRunId(run.getId()); step.setSequenceNo(stepRepository.findMaxSequenceNo(run.getId()) + 1); step.setType(type); step.setSummary(summary); step.setDetailJson(json(detail)); step.setCreatedAt(Instant.now()); step = stepRepository.save(step); try { messagingTemplate.convertAndSend("/topic/agent-cases/" + run.getCaseId(), new AgentDtos.AgentEvent(step.getId(), run.getCaseId(), run.getId(), type, step.getSequenceNo(), step.getSummary(), step.getCreatedAt())); } catch (Exception ex) { log.warn("Failed to send agent WebSocket event for case={}", run.getCaseId(), ex); } }
  private String json(Object value) { try { return objectMapper.writeValueAsString(value); } catch (Exception ex) { throw new IllegalStateException("动作审计序列化失败", ex); } }
  private String abbreviate(String value, int max) { if (value == null) return "动作执行失败"; return value.length() <= max ? value : value.substring(0, max); }
}
