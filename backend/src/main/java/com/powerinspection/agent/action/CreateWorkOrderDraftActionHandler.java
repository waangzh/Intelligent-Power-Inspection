...
public class CreateWorkOrderDraftActionHandler implements AgentActionHandler {
  ...
  public CreateWorkOrderDraftActionHandler(
  ...
  public boolean supports(AgentEnums.ActionType type) {
    return type == AgentEnums.ActionType.CREATE_WORK_ORDER_DRAFT;
  }
  ...
  public Map<String, Object> execute(AgentActionEntity action, UserEntity user) {
    ...
    if ((alarmId == null && taskId == null)
        || (alarmId != null && !alarmId.equals(agentCase.getAlarmId()))
        || (taskId != null && !taskId.equals(agentCase.getTaskId())))
      throw ApiException.badRequest("工单动作对象不属于当前案件");
    Map<String, Object> existing =
        dataStore.list(DataCategory.WORK_ORDER).stream()
            .filter(
                order ->
                    action.getId().equals(text(order.get("agentActionId")))
                        || action
                            .getIdempotencyKey()
                            .equals(text(order.get("agentIdempotencyKey"))))
            .findFirst()
            .orElse(null);
    ...
    boolean related =
        dataStore.list(DataCategory.WORK_ORDER).stream()
            .anyMatch(
                order ->
                    (alarmId != null && alarmId.equals(text(order.get("alarmId"))))
                        || (taskId != null && taskId.equals(text(order.get("taskId")))));
    if (related) throw ApiException.conflict("关联对象已存在工单");
    payload.put("agentActionId", action.getId());
    payload.put("agentIdempotencyKey", action.getIdempotencyKey());
    Map<String, Object> created = toolService.createWorkOrderDraft(payload, user);
    return result(created, "已创建工单草稿");
  }

  private Map<String, Object> result(Map<String, Object> order, String summary) {
    return Map.of(
        "resourceType",
        "WORK_ORDER",
        "resourceId",
  ...
  private Map<String, Object> payload(String json) {
    try {
      ...
      throw ApiException.badRequest("工单动作参数损坏");
    }
  }
