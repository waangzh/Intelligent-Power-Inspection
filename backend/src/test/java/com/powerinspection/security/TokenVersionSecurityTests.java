...
class TokenVersionSecurityTests {
  ...
  void disabledUserCannotAccessReadApisWithOldAccessToken() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/sites").header("Authorization", "Bearer " + viewerToken))
      .andExpect(status().isOk());

    try {
      mockMvc.perform(patch("/api/v1/users/" + viewerId + "/enabled")
      ...
      mockMvc.perform(get("/api/v1/auth/me").header("Authorization", "Bearer " + viewerToken))
        .andExpect(status().isUnauthorized());
      mockMvc.perform(get("/api/v1/notifications").header("Authorization", "Bearer " + viewerToken))
        .andExpect(status().isUnauthorized());
    } finally {
      mockMvc.perform(patch("/api/v1/users/" + viewerId + "/enabled")
          .header("Authorization", "Bearer " + adminToken)
  ...
  void passwordChangeInvalidatesPreviousAccessToken() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/auth/me").header("Authorization", "Bearer " + oldToken))
      .andExpect(status().isUnauthorized());
    mockMvc.perform(get("/api/v1/sites").header("Authorization", "Bearer " + oldToken))
      .andExpect(status().isUnauthorized());
