...
class RouteRevisionControllerTests {
  ...
  @Autowired RouteRevisionRepository routeRevisionRepository;
  ...
  void draftValidationOverwritesMapIdentityReportsAllIssuesAndDoesNotPersist() throws Exception {
    ...
    String routeBefore =
        objectMapper.writeValueAsString(dataStore.get(DataCategory.ROUTE, routeId));
    ...
    String normalized =
        mockMvc
            .perform(
                post("/api/v1/routes/{id}/draft:validate", routeId)
                    .header("Authorization", bearer(token))
                    .contentType(MediaType.APPLICATION_JSON)
            ...
            .andExpect(
                jsonPath("$.data.normalizedExecutorJson.map.image_sha256")
                    .value("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"))
            .andExpect(
                jsonPath("$.data.normalizedExecutorJson.map.vendor_extension").value("preserve"))
            .andReturn()
            .getResponse()
    ...
    assertEquals(
        routeBefore, objectMapper.writeValueAsString(dataStore.get(DataCategory.ROUTE, routeId)));

    Map<String, Object> invalid = validV3Executor();
    @SuppressWarnings("unchecked")
    Map<String, Object> startLocation =
        (Map<String, Object>) ((Map<String, Object>) invalid.get("start_pose")).get("location");
    ...
    for (var issue : objectMapper.readTree(invalidResponse).path("data").path("issues"))
      pointers.add(issue.path("jsonPointer").asText());
  ...
  void v2DraftWithMapAssetCreatesStableV3Revision() throws Exception {
    ...
    String first =
        mockMvc
            .perform(
                post("/api/v1/routes/{id}/revisions", routeId)
                    .header("Authorization", bearer(token)))
            .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            post("/api/v1/routes/{id}/revisions", routeId).header("Authorization", bearer(token)))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            post("/api/v1/route-revisions/{id}/deployments", revisionId)
                .header("Authorization", bearer(token))
    ...
    mockMvc
        .perform(
            post("/api/v1/tasks/task_route_revision/dispatch")
                .header("Authorization", bearer(token)))
        .andExpect(status().isConflict());

    mockMvc
        .perform(
            patch("/api/v1/tasks/task_route_revision")
                .header("Authorization", bearer(token))
                .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc
        .perform(
            post("/api/v1/tasks/task_route_revision/pause")
                .header("Authorization", bearer(token))
                .header("Idempotency-Key", "pause-route-revision"))
        .andExpect(status().isConflict());

    mockMvc
        .perform(delete("/api/v1/tasks/task_route_revision").header("Authorization", bearer(token)))
        .andExpect(status().isOk());

    mockMvc
        .perform(delete("/api/v1/routes/{id}", routeId).header("Authorization", bearer(token)))
        .andExpect(status().isOk());

    mockMvc
        .perform(delete("/api/v1/map-assets/{id}", mapId).header("Authorization", bearer(token)))
        .andExpect(status().isConflict());
  }
  ...
  void persistedDraftNormalizesRetainsLastPublishableReportAndChecksVersion() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/routes/{id}/draft", routeId)
                .header("Authorization", bearer(token))
                .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc
        .perform(get("/api/v1/routes/{id}/draft", routeId).header("Authorization", bearer(token)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.normalizedExecutorJson.map.yaml").value("floor.yaml"))
    ...
    mockMvc
        .perform(
            get("/api/v1/routes/{id}/draft:check", routeId).header("Authorization", bearer(token)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.valid").value(false))
        .andExpect(jsonPath("$.data.publishable").value(false));
        ...
        "routes",
            List.of(
                map(
                    ...
                    true,
                    "loop",
                    map("enabled", false, "wait_sec", 0, "max_cycles", 0),
                    "goal_timeout_sec",
                    120,
